...
class Login_users_resource(restful.Resource):
	...
	def post(self):
		...
		if user is None or not Users.verify(password, user.hashedPass):
			return {"message": "Error: Username or password is incorrect."}, 403, {'Access-Control-Allow-Origin': '*'} 
...
class Create_user_resource(restful.Resource):
	...
	def post(self):
		try:
			...
			data.session.commit()
			return {"message": "User with email {0} created".format(email), "user_id": newUser.id}, 201, {'Access-Control-Allow-Origin': '*', "Access-Control-Allow-Methods": "GET, POST"} 
