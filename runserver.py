...
class Create_user_resource(restful.Resource):
	...
	def post(self):
		try:
			...
			data.session.commit()
			return {"message": "User with email {0} created".format(email), "user_id": newUser.id}, 201, {'Access-Control-Allow-Origin': 'http://healthforum.herokuapp.com/registration/user', "Access-Control-Allow-Methods": "GET, POST"} 
